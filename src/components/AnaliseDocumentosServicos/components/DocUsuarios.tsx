...
export default function DocUsuarios({pessoa, handleInput, lists, docLists, loading, quantidade, processData, url} : IDocUsuario) {   
    ...
    const openPessoa = () => {
        localStorage.setItem('tab_select', pessoa === 'vendedor' ? '2' : '3');
        router.push(`/${url}/${processData?.imovel?.processo_id}/detalhes-venda`)
    }
            ...
            <section className='cards docs servicos'>
                <div className='title' style={{justifyContent: 'space-between'}}>
                    <div className='info' style={{display: 'flex', alignItems: 'center', gap: '4px'}}>
                        ...
                        {pessoa === 'vendedor' ? <SellerIco className="sucess" height={24} /> : <BuyerIco className="sucess" height={24} />}
                        <h2>{pessoasTitle} {Number(quantidade) > 1 && <Chip className='chip neutral' label={`${quantidade}`} />}</h2>
                ...
                <div className='list-items list-doc-analise' style={{paddingBottom: '0px'}}>
                    {loading
                        ? <SkeletonDocumentos />
                        : docLists?.data?.map((doc =>
                            <div className='item' key={doc.id}>
                                ...
                                <div className='icon-label actions actions-doc'>
                                    <ButtonComponent size={'small'} variant={'outlined'} startIcon={''} label={'Ver documento'} onClick={() => ShowDocument(doc.id, 'documento')} />
                                </div>
