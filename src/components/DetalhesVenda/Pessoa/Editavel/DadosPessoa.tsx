...
import { DataArray, Troubleshoot } from '@mui/icons-material';
...
export default function DadosPessoa ({ handleNextBlock, handlePrevBlock, index, data, Footer, handleShow, setBlockSave, saveBlocks, setUserData, type, blocksLength, userData }: BlockProps) {
  ...
  const tipo = type === 'vendedores' ? 'vendedor' : 'comprador';
  ...
  const defaultValues = {
    ...
    socio: usuario?.pj_socio === 1 ? true : false,
    representante: usuario?.pj_representante === 1 ? true : false
  };
    ...
    const returnDataPJ = async () => {
      if (dataUrl.user === 'pessoa-juridica' && dataUrl.index[1] === "representante") {
        ...
        if (quantidadeRep === 0 || verificaSocioRep === true) {
          ...
          setCheckedSocio(usuario?.pj_socio === 1 ? true : false);
          setCheckedRepresentante(usuario?.pj_representante === 1 ? true : false);
        }

        setQtdRep(quantidadeRep);
      }
      else {
        setCheckedRepresentante(watch('representante'));
        setCheckedSocio(watch('socio'));
      }
    }
    returnDataPJ()
  ...
  const save = () => {
    let valor: any = {
      'bloco': 0/*index*/,
      'processo_id': idProcesso !== 0 ? idProcesso : data.processo_id,
      'usuario_id': watch('usuario_id'),
      'pj_representante_id': userData?.tipo_pessoa === 0 ? userData?.vinculo_empresa?.id : '',
      'tipo_usuario': tipo, //vendedor ou comprador
  ...
  const handleCpf = async (value: any) => {
    if (value.length === 14) {
      ...
      setUserData && setUserData(dataCpf);
  ...
  const handleInput = (type: any, value: string) => {
    ...
    if (type === 'cpf') {
      ...
      const userId = usuario?.cpf_cnpj !== watch('cpf') ? '' : usuario?.id;
      setValue('usuario_id', userId);
    }
        ...
        <div className={styles.headerBlock}>
          {
            perfil === 'Pós-venda'
              ...
              : <>
                <h3>E quem está {type === 'vendedores' ? 'vendendo' : 'comprando'} o imóvel?</h3>
        ...
        <div className="content">
                ...
                {socioRepresentanteCheck.map(({ index, label, value, path, checked }) => (
                  <CheckBox
                    ...
                    {...register(index === 0 ? 'representante' : 'socio', {
                      required: (userData?.vinculo_empresa?.id)
                        ? watch('socio') === true
                          ? false
          ...
          <div className="row-f">
            <InputText
              ...
              onBlurFunction={() => handleCpf(watch('cpf'))}
              {...register('cpf', {
                //required: true,
                required: errorMsg,
          ...
          <div className="row-f">
            ...
            <InputText
              ...
              saveOnBlur={() => validarEmail(watch('email'))}
            />
          </div>
          ...
          <div style={{ marginBottom: 5 }}>
            {openSecond && (
              ...
              >
                ...
                Utilize seu CPF apenas se você for <b>{tipo === 'vendedor' ? 'vender' : 'comprar'}</b> esse imóvel.
              </Alert>
            )}
          </div>
          {open && (
