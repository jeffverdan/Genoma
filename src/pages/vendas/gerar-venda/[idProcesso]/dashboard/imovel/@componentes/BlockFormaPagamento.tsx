...
const BlockPage: React.FC<BlockProps> = ({ handleNextBlock, handlePrevBlock, index, data, Footer }) => {
  ...
  const formasDePagamentos = [
    ...
    { index: 2, label: "FGTS", value: "3", path: "formaPagamento.fgts", checked: checkedFgts, disabled: false },
    ...
    { index: 4, label: "Parcelamento", value: "5", path: "formaPagamento.parcelamento", checked: checkedParcelamento, disabled: false },
    //{ index: 5, label: "PIX", value: "6", path: "formaPagamento.pix", checked: checkedPix, disabled: false },
  ]
  ...
  const handleFormaPagamento = (e: any) => {
    ...
    // Criando um array para armazenar os valores selecionados
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const valueFormat = formatoMoeda(value);
    console.log(name, value, valueFormat);
    setValue(name, valueFormat.toString());    
  };
        ...
        <div className="content">
          <div className="row-f">
            <InputText
              ...
              required={true}
              value={watch('valorAnunciado')}
              sucess={!errors.valorAnunciado && !!watch('valorAnunciado')}
              onBlurFunction={() => handleInput('valorAnunciado')}
              {...register('valorAnunciado', {
                required: errorMsg,
                onChange: (e) => onChange(e),
                // setValueAs: (value: string) => formatoMoeda(value)
              })}
            />
            ...
            <InputText
              ...
              value={watch('valorVenda')}
              sucess={!errors.valorVenda && !!watch('valorVenda')}
              onBlurFunction={() => handleInput('valorVenda')}
              {...register('valorVenda', {
                required: errorMsg,
                // onChange: (e) => handleInput('valorVenda'),
                setValueAs: (value: string) => formatoMoeda(value)
              })}
            />
            ...
            <InputText
              ...
              value={watch('valorSinal')}
              sucess={!errors.valorSinal && !!watch('valorSinal')}
              onBlurFunction={() => handleInput('valorSinal')}
              {...register('valorSinal', {
                required: errorMsg,
                // onChange: (e) => handleInput('valorSinal'),
                setValueAs: (value: string) => formatoMoeda(value)
              })}
            />
          </div>
              ...
              {formasDePagamentos.map(({ index, label, value, path, checked, disabled }) => (
                <CheckBox
                  ...
                  {...register('formasPagamento', {
                    required: true,
                    onChange: (e) => [handleFormaPagamento(e), handleInput('formasPagamento')],
                  })}
                />
              ))}
            </div>
          ...
          <div style={{ margin: '50px 0' }}>
            <TextArea
              ...
              {...register('observacaoPagamento', {
                required: false,
                onChange: (e) => [setValue('observacaoPagamento', e.target.value), handleInput('observacaoPagamento')]
              })}
            />
          </div>
        </div>
