...
type CheckType = "vendedor_pf" | "comprador_pf"
...
const ReviewPF = (props: Props) => {
    const { checks, setChecks, list, type, pessoas, saveData, error, readyOnly, returnData } = props;
    const { register, watch, setValue } = useForm();
    const [collapse, setCollapse] = useState(false);
    const checkType: CheckType = type ? `${type}_pf` : 'vendedor_pf'
    ...
    const emptyList = { // CRIA O PLACEHOLDER DA LISTA
        id: '',
        name: `Selecione o ${type}`,
    };
    const [listPessoas, setListPessoas] = useState<ListType[]>([emptyList]);
    ...
    useEffect(() => {
        setListPessoas([emptyList, ...pessoas.map((pessoa) => ({ id: pessoa.id, name: pessoa.name }))]);
    ...
    const handleCheck = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const tipo = type === 'vendedor' ? { vendedor_pf: check } : { comprador_pf: check };
            ...
            <div className='card-review'>
                <div className='header-card' onClick={() => setCollapse((prev) => !prev)}>
                    <FormControlLabel
                        ...
                        label={`Revisar cadastro de ${type}es (pessoa física)`}
                ...
                <div className='content-card'>
                    ...
                    {selectedPessoas.map((pessoa, index) => (
                        <div className='content-pessoa' key={index}>
                            ...
                            <MultipleSelectCheckmarks
                                ...
                                labelMenu={`Correções do ${type} - PF`}
                                maxWidth={300}
                                placeholder='Ex: Documentos, endereço, procurador...'
                            />
                        </div>
                    ))}
                    ...
                    <div className='text-area'>
                        <EmptyTextarea
                            ...
                            label={`Observações de ${type}es - pessoa física`}
                            placeholder={`Ex: As informações dos ${type}es não condizem com os documentos.`}
                            {...register("obs", {
